...
The regulatory window is open and dated. eIDAS 2.0 (Regulation (EU) 2024/1183) requires every EU member state to offer a government-backed EU Digital Identity Wallet by December 2026; the Architecture and Reference Framework and its implementing acts continue to be published and refined through 2026, and relying-party obligations follow in the period after. A BSV identity layer that is standards-aligned can interoperate with this infrastructure rather than compete with it.

The specific gap this work targets is unlinkability. The EUDI mainstream selective-disclosure format (SD-JWT) is linkable across colluding verifiers, and the framework does not yet resolve privacy-preserving presentation. Unlinkable-yet-verifiable presentation — the pattern in §10, delivered by the toolbox in §13 — is therefore the differentiator, not a refinement. The advantage is realised on the BSV-native and zero-knowledge path; presentations made in the EU-baseline formats (SD-JWT VC, mdoc) inherit those formats' linkability until the ARF's zero-knowledge track matures (§19.4).
...
- *Personhood nullifier `n` (uniKey)* — a deterministic, one-way function of a person's passport identity (Tier 1); the basis for **per-document** global uniqueness (one account per identity document). It is **not** secret — anyone who has read the passport chip can recompute it — so it surfaces only as further one-way, scope-bound nullifiers, and pseudonyms are never derivable from it (§8.4, §10.7).
...
- **P3 — Unlinkable across contexts.** Distinct RPs MUST receive uncorrelated identifiers; *cryptographic* correlation MUST remain infeasible even if RPs collude. This requirement is scoped to the cryptographic layer: it does not constrain network-level correlation (§20.2) or correlation by a custodial wallet (§8.5), which are addressed separately, and it is qualified by the linkability inherent in any published global-uniqueness set (§10.7).
- **P4 — Deliberate uniqueness.** Where one-person-one-account is required, the scope of uniqueness (per-RP vs global) MUST be stated explicitly; global uniqueness requires either a shared nullifier scope or an accountable one-credential-per-person guarantee.
...
- **Authentication and presentation.** BRC-103 is a peer-to-peer mutual authentication and certificate-exchange protocol built on Schnorr signatures, nonce challenges, and BRC-42 derivation. It supports selective field disclosure through per-field keyrings, on-chain (UTXO outpoint) revocation, and interchangeable transports — HTTP, NFC, or WebSocket.
- **Credentials.** BRC-52 defines privacy-centric identity certificates with selective revelation and UTXO-based revocation: fields are individually encrypted (BRC-2) and signed (BRC-3), and disclosed to a verifier through a keyring that is not itself part of the signature. BRC-53 adds the request, creation, and revelation methods that let applications obtain and interact with those certificates over the BRC-1 messaging layer.
...
<svg viewBox="0 0 760 424" width="760" height="424" style="max-width:100%;height:auto" xmlns="http://www.w3.org/2000/svg" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif">
  <defs><marker id="f1arr" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M0,0 L10,5 L0,10 z" fill="#64748b"/></marker></defs>
  ...
  <text x="380" y="69" text-anchor="middle" font-size="13" fill="#4c1d95">eIDAS / EUDI interoperability profile (Part IV) · PID · QEAA · Trusted Lists</text>
  <line x1="380" y1="88" x2="380" y2="104" stroke="#64748b" stroke-width="1.5" marker-end="url(#f1arr)"/>
  ...
  <line x1="380" y1="152" x2="380" y2="168" stroke="#64748b" stroke-width="1.5" marker-end="url(#f1arr)"/>
  ...
  <line x1="200" y1="226" x2="200" y2="250" stroke="#64748b" stroke-width="1.5" marker-end="url(#f1arr)"/>
  <line x1="560" y1="226" x2="560" y2="250" stroke="#64748b" stroke-width="1.5" marker-end="url(#f1arr)"/>
  ...
  <line x1="200" y1="314" x2="200" y2="336" stroke="#64748b" stroke-width="1.5" marker-end="url(#f1arr)"/>
  <line x1="560" y1="314" x2="560" y2="336" stroke="#64748b" stroke-width="1.5" marker-end="url(#f1arr)"/>
...
- the *personhood nullifier* `n` (the uniKey) — a deterministic, one-way function of the holder's passport identity attributes at Tier 1 (document number, MRZ, DG2 facial-image hash). It anchors **per-document** global uniqueness (§10.7): the same passport always yields the same `n`, even from a fresh wallet, so uniqueness is enforced as *one account per identity document* — a renewed passport or a second lawful document each yields a fresh valid `n` (§12), making this strong Sybil resistance rather than provable one-human-one-account. Crucially, `n` is **not** a secret — anyone who has read the passport chip can recompute it — so it MUST be used only to derive further one-way, scope-bound nullifiers, and scoped pseudonyms MUST NOT be derivable from `n`. A direct consequence (§10.7, §20.4): a *global* uniqueness nullifier keyed only on `n` would be deterministic but also recomputable by any party who has read the chip. The standard therefore keys the published global nullifier on a registry-held oblivious-PRF secret rather than on `n` alone (§10.7.1 path B, C.4.6), which restores brute-force resistance and unrecomputability while preserving per-passport determinism for de-duplication.
...
<svg viewBox="0 0 720 348" width="720" height="348" style="max-width:100%;height:auto" xmlns="http://www.w3.org/2000/svg" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif">
  <defs><marker id="f4arr" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M0,0 L10,5 L0,10 z" fill="#64748b"/></marker></defs>
  <rect x="0" y="0" width="720" height="348" rx="10" fill="#ffffff"/>
  <text x="24" y="26" font-size="15" font-weight="700" fill="#0f172a">Figure 4 · Unlinkable KYC — one person, a different identifier per service</text>
  ...
  <line x1="144" y1="100" x2="144" y2="126" stroke="#64748b" stroke-width="1.5" marker-end="url(#f4arr)"/>
  ...
  <line x1="144" y1="180" x2="144" y2="206" stroke="#64748b" stroke-width="1.5" marker-end="url(#f4arr)"/>
  ...
  <path d="M260,234 C360,234 360,107 466,107" fill="none" stroke="#64748b" stroke-width="1.5" marker-end="url(#f4arr)"/>
  <path d="M260,234 C360,234 360,179 466,179" fill="none" stroke="#64748b" stroke-width="1.5" marker-end="url(#f4arr)"/>
  <path d="M260,234 C360,234 360,251 466,251" fill="none" stroke="#64748b" stroke-width="1.5" marker-end="url(#f4arr)"/>
  <text x="360" y="324" text-anchor="middle" font-size="12" fill="#475569">Every presentation is verifiable; the pseudonyms stay unlinkable across services — correlation remains infeasible even under collusion.</text>
</svg>
...
(e) `pid = PRF(s, r)`;
revealing to the relying party only `pid`, the truth of `P`, and the satisfied tier — and nothing else.
...
**10.6 Currency and revocation.** The proof MUST bind to credential and DID status (§7.5) such that a revoked or expired credential cannot produce an accepted presentation. Where §10 unlinkability is required, status MUST be checkable by the relying party without contacting the issuer (P8), and the privacy-preserving accumulator mechanism of §13.5 (non-membership in a published accumulator) MUST be used; the issuer-fetched status-list profiles permitted in C.3 leak the timing of the check and therefore MUST NOT be the sole status mechanism on the unlinkable path. Where unlinkability is not required, an issuer-contacting status list (C.3) MAY be used.

**10.7 Uniqueness scope — the deliberate choice (P4).** Per-RP uniqueness — the scoped pseudonym `pid = PRF(s, r)` — gives Sybil resistance at each relying party with full cross-RP privacy and needs no shared infrastructure, because `pid` is stable for that RP yet uncorrelated across RPs. Global one-person-one-account is harder and rests on the personhood nullifier `n` (§8.4), which is deterministic from the passport and so survives re-enrolment with a fresh `s`. A deployment MUST state which uniqueness guarantee it offers — per-RP, or global — and any published set MUST contain only nullifiers, never identifying data (P12).

**10.7.1 The global-dedup constraint.** Global de-duplication needs a *shared comparison surface*: two enrolments of the same passport MUST collide on the same value. Making that surface resistant to offline brute-force needs the colliding value to depend on a *high-entropy secret*; and a secret reproducible from "the same passport, from a fresh wallet, with no prior state" MUST be held by some party other than the wallet. It follows that **global, private, brute-force-resistant de-duplication cannot also be registry-free.** The naïve construction `gnull = PRF_n(domain)` (a hash keyed only on `n`) attempts all three and therefore fails: because `n` is low-entropy and recomputable by anyone who has read the chip (§§8.4, 20.4), such a value is brute-forceable and recomputable by any prior document-handler, and a deployment MUST NOT publish or rely on it. ICAO 9303 does not rescue the registry-free case: Active Authentication mixes a chip-chosen nonce into every signature (so it yields no reproducible chip secret) and Chip Authentication's shared secret is derivable from the readable DG14 static key, so the chip provides anti-clone *authentication*, not a reproducible secret.

A conforming deployment that offers global uniqueness MUST therefore implement one of:

- **(B) Blinded, rate-limited, anti-clone-gated nullifier (normative path, C.4).** A *Uniqueness Registry* — operated as a threshold-of-N committee holding a versioned verifiable-OPRF key `k_e` — supplies the high-entropy secret without learning `n` or any identifying data. The nullifier is `gnull_e = H("bsv-gnull" ‖ domain ‖ POPRF_{k_e}(domain; n))`, evaluated through the oblivious protocol of C.4.6. Registry evaluation MUST be gated on a live active/chip-authentication proof (§12) and rate-limited, so that only a party presenting the genuine physical chip can obtain `gnull_e`. This blocks offline brute-force (the secret `k_e` is required), blocks recomputation from copied data groups (a data copier cannot pass active authentication), and keeps the registry blind to identity (OPRF blinding, threshold key, verifiable evaluation). The residual exposure is a party in physical possession of the chip during a live session — the same borrowed/stolen-passport limit Tier 1 already carries (§12), narrowed from "anyone who ever saw the data" to "someone running a full chip session."
- **(A) Accountable one-credential-per-person (conformant fallback).** A Tier-2/3 issuer or qTSP enforces one credential per person at proofing time; no public nullifier set is published. A deployment that cannot run a registry, or for which global uniqueness is acceptable only at accountable tiers, MUST restrict global uniqueness to this path and MUST describe Tier 1 as providing per-RP uniqueness only.

Both paths remain *per-document*, not per-natural-person (§§8.4, 12), and the published global set makes a holder linkable *within its de-duplication domain* — the unavoidable privacy cost of global dedup, bounded per `domain` by C.4's per-domain key namespace. Tier 1's holder binding is self-asserted, so a Tier-1 global-uniqueness guarantee resists casual Sybil attacks but not determined impersonation with a borrowed or stolen passport (§12); the accountable guarantee comes from Tiers 2–3.

**10.8 Requirements summary.** A conforming implementation MUST: derive pseudonyms with a one-way PRF (§9.2); keep cross-scope correlation infeasible under collusion; disclose no attribute beyond the requested predicates; carry the tier as a provable attribute; bind every presentation to credential/DID status; never expose the root DID (§7.7); and limit any on-chain footprint to commitments, revocation handles, and nullifier sets.
...
<svg viewBox="0 0 760 232" width="760" height="232" style="max-width:100%;height:auto" xmlns="http://www.w3.org/2000/svg" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif">
  <defs><marker id="f3arr" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M0,0 L10,5 L0,10 z" fill="#64748b"/></marker></defs>
  ...
  <line x1="152" y1="80" x2="168" y2="80" stroke="#64748b" stroke-width="1.5" marker-end="url(#f3arr)"/>
  ...
  <line x1="298" y1="80" x2="314" y2="80" stroke="#64748b" stroke-width="1.5" marker-end="url(#f3arr)"/>
  ...
  <line x1="444" y1="80" x2="460" y2="80" stroke="#64748b" stroke-width="1.5" marker-end="url(#f3arr)"/>
  ...
  <line x1="590" y1="80" x2="606" y2="80" stroke="#64748b" stroke-width="1.5" marker-end="url(#f3arr)"/>
  ...
  <line x1="234" y1="112" x2="234" y2="150" stroke="#94a3b8" stroke-width="1.2" stroke-dasharray="4 3" marker-end="url(#f3arr)"/>
  <line x1="672" y1="112" x2="672" y2="150" stroke="#94a3b8" stroke-width="1.2" stroke-dasharray="4 3" marker-end="url(#f3arr)"/>
...
**11.2 Data model.** Claims SHOULD use established vocabularies (W3C / schema.org). Each attribute MUST be individually addressable so it can be selectively disclosed, or proven by predicate, without revealing its neighbours. Every credential carries, as verifiable metadata: its tier / level of assurance (§12), its attesting source, a validity period, a status/revocation pointer (11.5), and the commitments to `s` and `n`.
...
**11.5 Status, revocation, and source trust.** A credential's status MUST be checkable by a verifier via a `did:bsv` status operation (§7.5) or non-membership in a published revocation accumulator (§13.5); where §10 unlinkability is required, the check MUST be performable without contacting the issuer (§10.6), which the accumulator path satisfies. For issuer-issued credentials the attesting source is itself identified by a `did:bsv` and trusted through accreditation (Trusted Lists, §17); a presentation SHOULD be able to prove the source belongs to an accepted set without naming it (§13.4). Self-issued Tier 1 credentials have no issuer revocation; reliance is limited to document validity, expiry, and any published lost/stolen signals, consistent with the Tier 1 assurance ceiling (§12).

**11.6 Holder binding and non-transferability.** A credential MUST be non-transferable: presentation MUST require proof of control of the holder key **and** knowledge of `s`, so that neither a copied credential on a compromised key nor knowledge of `s` alone suffices for another party to present.
...
<svg viewBox="0 0 760 300" width="760" height="300" style="max-width:100%;height:auto" xmlns="http://www.w3.org/2000/svg" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif">
  <defs><marker id="f2arr" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M0,0 L10,5 L0,10 z" fill="#64748b"/></marker></defs>
  ...
  <line x1="188" y1="160" x2="204" y2="160" stroke="#64748b" stroke-width="1.5" marker-end="url(#f2arr)"/>
  ...
  <line x1="372" y1="150" x2="388" y2="150" stroke="#64748b" stroke-width="1.5" marker-end="url(#f2arr)"/>
  ...
  <line x1="556" y1="140" x2="572" y2="140" stroke="#64748b" stroke-width="1.5" marker-end="url(#f2arr)"/>
...
- *Layer D — Predicate + unlinkability + personhood (medium-term, SNARK-based).* A single succinct proof of the full §10 statement: a credential is validly attested, predicates hold, and `pid = PRF(s, scope)` for a committed `s`. Instantiation: a zk-SNARK/STARK with a SNARK-friendly PRF (e.g. Poseidon) for the scoped pseudonym `pid` (and, where global uniqueness is required, the §10.7.1 nullifier `gnull`) and, for Tier-1 self-read credentials, a circuit that verifies the ICAO 9303 passive-authentication signature chain in-circuit — yielding a private "genuine passport, predicates hold, scoped pseudonym `pid`" proof. This is the cryptographic engine of free, private, verifiable KYC, following the emerging class of zero-knowledge passport circuits.
...
**13.7 Where proofs are verified.** Presentation proofs are verified off-chain by the relying party. BSV anchors commitments, credential status/revocation, and — for global uniqueness — nullifier sets. On-chain data MUST remain limited to commitments, hashes, and nullifiers (P11, P12); no proof or attribute is required to be verified in Bitcoin script.
...
**14.4 Channels and replay protection.** Presentations run over web (same-device redirect or cross-device QR / deep link), WebSocket or HTTP (BRC-103), and NFC for in-person/proximity use (mdoc proximity or BRC-103 over NFC). Every presentation proof MUST be bound to a fresh verifier-supplied challenge (nonce) so it cannot be replayed, and the transport MUST be mutually authenticated.
...
<svg viewBox="0 0 760 270" width="760" height="270" style="max-width:100%;height:auto" xmlns="http://www.w3.org/2000/svg" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif">
  <defs><marker id="f5arr" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M0,0 L10,5 L0,10 z" fill="#64748b"/></marker></defs>
  ...
  <line x1="240" y1="150" x2="264" y2="150" stroke="#64748b" stroke-width="1.5" marker-end="url(#f5arr)"/>
  ...
  <line x1="482" y1="139" x2="506" y2="139" stroke="#64748b" stroke-width="1.5" marker-end="url(#f5arr)"/>
...
**19.4 Unlinkability reconciliation.** The Regulation mandates unlinkability in principle — Recital 14 calls for integrating privacy-preserving technologies such as zero-knowledge proof, and the technical framework forbids any party from obtaining data that lets transactions be tracked, linked, or correlated without the user's authorisation. The ARF's own definition of full unlinkability — no party, including a provider colluding with a relying party, can tell that disclosed attributes describe the same user — coincides with the §10 goal. But the ARF baseline formats (SD-JWT VC, mdoc) are linkable and reach verifier-unlinkability only by issuing single-use credentials in batches, at operational cost; anonymous-credential schemes (BBS+, and the pairing-free, ECDSA-compatible BBS#) are not yet defined in the ARF spec, though compatible with it, and are catalogued in ETSI TR 119 476. The eIDAS framework permits Member States to adopt more innovative ZKP schemes voluntarily. This profile therefore: (a) for the EU baseline path, emits ARF-compatible SD-JWT VC / mdoc, accepting their linkability or using batch issuance; and (b) offers the §13 Layer C/D zero-knowledge path as the privacy-preserving option aligned with Recital 14 and ETSI TR 119 476, positioned to map onto the ARF's ZKP track (BBS#/BBS+) as it matures. On unlinkability the layer's zero-knowledge path is ahead of the EU baseline; its baseline-format path inherits the same linkability as the EU baseline until the ARF zero-knowledge track matures, so the advantage holds only where the Layer C/D path is actually used (§22).
...
**20.4 Commitment confidentiality.** Any on-chain commitment MUST be blinded with high-entropy salt so it cannot be brute-forced or dictionary-attacked back to its inputs. This matters most for the personhood nullifier `n`, which is *low-entropy by construction* — recomputable by anyone who has read the passport — and therefore MUST never be committed or published except as a salted, one-way value; genuine entropy comes from the presentation secret `s` (§8.4). Treating `n` as if it were secret is the failure mode to avoid.

There is one mechanism this rule cannot fully protect, and the conflict MUST be stated rather than glossed: the *global* uniqueness nullifier `gnull` (Appendix C.4) must be **deterministic** in `n` so that two enrolments of the same passport collide for duplicate detection — it therefore cannot carry a per-holder salt, and is recomputable by any party who has read the chip. Per-holder salting (which defeats dictionary attacks) and cross-holder determinism (which global de-duplication requires) are mutually exclusive for this one value. Consequently `gnull` is linkable and testable *within its de-duplication domain* by a prior passport-handler, and global-de-duplication privacy holds only against parties who have never read the chip. Document numbers are themselves low-entropy and partly structured, so a naïve `gnull` keyed only on `n` MUST NOT be relied upon. The standard resolves this by introducing the high-entropy secret a private dedup surface requires: the global nullifier is keyed on a registry-held oblivious-PRF secret `k_e` rather than on `n` alone (§10.7.1 path B, C.4.6). This makes `gnull_e` brute-force-resistant and unrecomputable from copied data, while remaining deterministic per passport for de-duplication; the residual linkability is confined to within each `uniqueness_domain`. Deployments that cannot operate such a registry MUST instead use the accountable path (§10.7.1 path A) and MUST NOT publish a `gnull` set.

**20.5 Replay and transfer.** Presentation proofs MUST bind to a fresh verifier challenge (§14.4), defeating replay. Non-transferability (§11.6) prevents a copied credential from being presented by another party.
...
**20.7 Trust-anchor compromise.** A compromised CSCA undermines Tier 1; a compromised issuer/qTSP undermines the credentials it signed; both are contained by accreditation and registry revocation (§15.6). A compromised ZK trusted setup could forge proofs; transparent-setup systems (Bulletproofs, STARKs) avoid this, and where a setup is required it MUST use a multi-party ceremony (§13.6). The **Uniqueness Registry** (§10.7.1 path B, C.4.6) is a trust anchor only for the global-dedup secret `k_e`, never for identity: OPRF blinding keeps `n` and all identifying data off it, so even a fully compromised registry learns no identity. Its compromise modes are bounded — recovery of `k_e` re-enables offline brute-force of `gnull` *for that epoch's set only* (mitigated by threshold key-holding and rotation, §C.4.6); a malicious registry attempting to key-tag or return garbage is caught by VOPRF verifiability against the publicly committed `k_e`; and a registry cannot fabricate uniqueness for a forged document because evaluation is gated on passive- plus active-authentication proofs it verifies but does not produce. A registry outage degrades availability of *new* global-uniqueness enrolments only; existing receipts and all per-RP uniqueness (§10.7) are unaffected.

**20.8 Residual limits, stated plainly.** Tier-1 binding is self-asserted: a lost or stolen passport could be self-read by another party, so Tier-1 personhood and uniqueness resist casual abuse, not determined impersonation (§§10.7, 12). And secp256k1 ECDSA/Schnorr are not post-quantum; migration is possible through DID key rotation (§7.4) as post-quantum signatures and proof systems mature, and is a tracked long-term item.

### 21. Reference implementation and conformance — *Normative*

**21.1 Reference stack.** A reference implementation SHOULD build on the existing BSV TypeScript stack: the SDK wallet client and BRC-103 authentication, BRC-52/53 certificates, the `did:bsv` resolver (overlay/Teranode-native, with WhatsOnChain as a driver, §7.6), and a pluggable zero-knowledge module exposing the §13 layers (Bulletproofs on secp256k1; BBS+/BBS# on their respective curves; a SNARK toolchain including the ICAO passport circuit). A deployment offering global uniqueness via §10.7.1 path B SHOULD additionally provide a threshold (V)(P)OPRF Uniqueness Registry (RFC 9497 ciphersuite; C.4.6) with published epoch-key commitments.

**21.2 Conformance vectors.** The standard MUST publish test vectors for: ECDSA and Schnorr over secp256k1 (§8.1); type-42 derivation (§8.2); scoped-pseudonym PRF derivation (§10); the §10.3 presentation statement for each ZK layer; revocation-accumulator non-membership (§13.5); the SD-JWT VC and mdoc export mappings (§19.1); and, where global uniqueness is offered, the C.4.6 blinded-nullifier derivation — POPRF blind/evaluate/unblind/finalize and `gnull_e` formation — with VOPRF correctness-proof verification.
...
| Unlinkability (multi-show / collusion) | native, collusion-resistant | baseline linkable; batch issuance | yes | n/a (identifier only) | none; provider sees all |
| Predicate / ZK proofs | yes (§13) | emerging (ARF ZKP topic) | yes | no | no |
...
**Phase 1 — Predicates and unlinkability.** Zero-knowledge Layers B–D: range/predicate proofs, unlinkable presentation, scoped nullifiers, and the ICAO passport circuit. This is the privacy differentiator. *Gate:* ZK audit (including any setup ceremony); unlinkability tested against pooled-RP collusion.
...
**Conclusion.** BSV already holds the pieces of a national-grade identity system — type-42 keys, BRC-103 authentication, BRC-52/53 selective-disclosure certificates, and a registered, UTXO-friendly `did:bsv` — but they have stood as two unconnected stacks. This proposal binds them into one standard whose defining property is KYC that is private, verifiable, and unlinkable: a person proves they are a real, current, appropriately-assured human who meets a relying party's conditions, presents a different identifier to every service, and — on the non-custodial, zero-knowledge path — cannot be cryptographically correlated across services even under collusion. Onboarding begins free, with a self-read passport, and rises through issuer-verified, in-person, and ultimately notified-eID assurance. The layer interoperates with eIDAS at every stage and, where its zero-knowledge (Layer C/D) path is used, leads the EU baseline on unlinkability — a roadmap capability (Phase 1, §23) rather than a property of the first shipping milestone; on the baseline-format path it inherits the same linkability as the EU baseline until the ARF's own zero-knowledge track matures. The next steps are to publish it as a BRC and an eIDAS interoperability profile, build the open reference implementation and conformance suite (§21), and engage a qTSP and a Member State for the qualified and notified stages.

**Appendix A — BRC reference.**

| BRC | Subject | Role here |
|---|---|---|
| 1 | Abstract messaging layer | carries certificate requests (BRC-53) |
...
| 53 | Certificate creation and revelation | issuance/revelation methods (§11) |
| 100 | Wallet-to-application interface | wallet API substrate (§5.1) |
...
```json
{
  ...
  "credentialSubject": {
    "personhoodCommitment": {
      "s_commitment": "<blinded commitment to presentation secret s>",
      "n_commitment": "<blinded one-way commitment to nullifier n>",
      "scheme": "pedersen-poseidon-v1"
    },
...
**C.4 Personhood commitment and nullifier (§8.4, §10).**
- *Presentation secret* `s` — high-entropy (≥128-bit), wallet-generated, domain-separated; recovered via §8.6, never from a document.
- *Personhood nullifier* `n` (the uniKey) — `n = H_dst("bsv-personhood-n" ‖ canonical(passport_identity))`, a one-way hash over the canonicalised passport identity (document number, MRZ fields, DG2 image hash). Deterministic and **not** secret.
- *Commitments in the credential* — `C_s = Commit(s; r_s)` and `C_n = Commit(n; r_n)`, blinded (§20.4); Pedersen on the proof-system group, or Poseidon inside a SNARK circuit.
- *Scoped pseudonym* — `pid = PRF_s(scope_RP)`, with a SNARK-friendly PRF (e.g. Poseidon) where proven in zero knowledge, or HMAC-SHA-256 where no proof is needed; `scope_RP` is the verifier's stable scope (§14.2). The PRF instantiation MUST be fixed per `scope_RP`: because Poseidon and HMAC-SHA-256 produce different outputs for the same `(s, scope_RP)`, a holder who presented sometimes in zero knowledge and sometimes in the clear to the *same* RP would otherwise yield two different `pid`s, breaking the per-RP stability that local one-account enforcement (§10.7) depends on. A given relying-party relationship MUST therefore use one PRF for the life of the relationship.
- *Global uniqueness nullifier* (when required, §10.7) — `gnull_e = H_dst("bsv-gnull" ‖ uniqueness_domain ‖ y)`, where `y = POPRF_{k_e}(uniqueness_domain; n)` is the output of a registry-held partially-oblivious PRF (C.4.6), and `e` is the registry key epoch. The naïve form `gnull = PRF_n(uniqueness_domain)` — keyed only on `n` — MUST NOT be used: because `n` is low-entropy and recomputable by anyone who has read the chip (§§8.4, 20.4), that form is brute-forceable and recomputable by any prior document-handler. Keying on the registry secret `k_e` removes both: `gnull_e` is deterministic in `(n, k_e)` (so de-duplication still collides for the same passport) yet computable only via the registry, which evaluates it solely behind a live active/chip-authentication gate and a rate limit. `gnull_e` remains linkable *within its `uniqueness_domain`*; distinct domains use disjoint key namespaces and are mutually unlinkable.
- Domain-separation tags MUST keep `s`-derived pseudonyms and `n`-derived nullifiers in disjoint spaces, so pseudonyms are never derivable from `n` (§8.4).

**C.4.6 Uniqueness Registry and oblivious evaluation (normative, §10.7.1 path B).** The Uniqueness Registry holds a verifiable, partially-oblivious PRF key, secret-shared across a threshold-of-N committee, and versioned by epoch `e` (`k_e`). The ciphersuite SHOULD be a RFC 9497 (V)(P)OPRF suite (e.g. ristretto255-SHA-512 or P-256-SHA-256) in a threshold realisation (2HashDH/Pythia-class). Enrolment / first-show:

1. The wallet performs ICAO 9303 **passive authentication** (Document Security Object signature, DSC→CSCA chain; §12) and **active or chip authentication** against a fresh registry challenge `chal` (anti-clone; §12).
2. The wallet computes `n` (above) and blinds it: `b = Blind(n; r)` for fresh blinding `r`.
3. The wallet sends the registry `{b, uniqueness_domain, chal-response, π}`, where `π` is a zero-knowledge proof that `b` blinds an `n` derived from the passive-authenticated passport (binding the OPRF input to a genuine document without revealing `n`).
4. The registry verifies passive auth, the live active-auth response, and `π`; applies rate-limiting (e.g. per issuing CSCA per time window); evaluates `B = Eval_{k_e}(uniqueness_domain, b)`; and returns `B` with a VOPRF proof of correct evaluation under the publicly committed epoch key `k_e`.
5. The wallet checks the VOPRF proof, unblinds to recover `y`, and forms `gnull_e`. The registry (or overlay) tests `gnull_e` for membership in the epoch-`e` set: present ⇒ duplicate; absent ⇒ insert. The registry then issues a signed *uniqueness receipt* binding `gnull_e` to the credential commitment `C_n`.

Required properties: the registry never receives `n` or any identifying data (OPRF blinding); no single operator holds `k_e` (threshold); the holder can detect a misbehaving or key-tagging registry (VOPRF verifiability against the published `k_e` commitment); offline brute-force is infeasible (the secret `k_e` is required); and recomputation from copied data groups is infeasible (step 1's live active-auth gate, which a data copier cannot satisfy). Subsequent presentations do **not** repeat the OPRF: the Layer-D proof (§13) proves knowledge of a valid uniqueness receipt binding `gnull_e` to the committed `n`, keeping the OPRF and passive/active-auth work a one-time enrolment cost. Key lifecycle: `k_e` MUST be rotated on suspected compromise (compromise re-enables brute-force for that epoch's set only); rotation re-namespaces `gnull`, so the registry MUST support dual-epoch evaluation across a migration window and map a holder's old-epoch entry to the new one at next presentation. Chips lacking active/chip authentication cannot satisfy step 1; for such documents the deployment MUST fall back to §10.7.1 path A and MUST NOT claim the anti-recomputation property.
...
*References:* [VCDM2] W3C Verifiable Credentials Data Model 2.0 (Recommendation, 2025-05-15); [SD-JWT-VC] IETF SD-JWT VC (draft-ietf-oauth-sd-jwt-vc); [SD-JWT] RFC 9901 (Selective Disclosure for JWTs); [MDOC] ISO/IEC 18013-5 and 18013-7; [BSL] W3C Bitstring Status List v1.0 (Recommendation, 2025-05-15); [TSL] IETF OAuth Token Status List (draft-ietf-oauth-status-list); [27560] ISO/IEC TS 27560:2023; [29184] ISO/IEC 29184:2020; [KCR] Kantara Consent Receipt v1.1; [DPV] Data Privacy Vocabulary; [OPRF] RFC 9497 (Oblivious Pseudorandom Functions using Prime-Order Groups). Commitment, PRF, accumulator, and oblivious-PRF constructions follow standard cryptographic literature (Pedersen commitments; Poseidon; AnonCreds; ALLOSAUR; threshold/verifiable OPRFs of the 2HashDH/Pythia class; rate-limiting nullifiers, RLN).
...
- *Personhood nullifier `n` (uniKey)* — a deterministic, one-way, passport-derived value anchoring **per-document** global uniqueness (one account per identity document, not provably one per person); not a secret, and recomputable by anyone who has read the chip (§8.4, §10.7, §20.4).
...
- *Personhood commitment* — a credential's blinded commitment to two values: the wallet-held presentation secret `s` (source of pseudonyms and holder binding) and the passport-derived nullifier `n` (the uniKey, anchor of uniqueness) (§8.4, §10).
