...
**8.10 Root-secret compromise and `s`-migration.** Two events must not be conflated. *Loss of a device* whose secure element holds the root secret is a recovery event (§8.6): the secret is non-exportable, an authenticator gates access (§8.8), and the device's keys are individually revocable — implementations MUST maintain an authorized-device list under the DID (§7.4) and support per-device revocation. *Exposure of the root secret itself* — extraction, provisioning-time malware, a broken secure element — is categorically different: `s` derives from it, `s` cannot be rotated without changing every pseudonym, and no key rotation makes an attacker forget a secret. This specification therefore defines the compromise path rather than claiming rotation contains it:
...
- Handover MUST be gated on the registry replacement: knowledge of the exposed `s` alone MUST NOT suffice, or the attacker could migrate the victim's accounts to keys the attacker controls. Because replacement under an existing tag requires a fresh Layer-D enrolment over the same physical document (or Tier-2/3 eviction, §12), only the party holding the document can complete migration; races resolve through accountable eviction (Appendix F.6).
...
**10.10 Delegated CDD custody — *Normative (optional profile)*.** A third presentation pattern between predicate-only (§10.3) and full disclosure (§14.3): the relying party with the legal CDD obligation (the *company*) never holds plaintext PII; a regulated *custodian* holds the customer-due-diligence record; the company holds a verifiable attestation of due diligence, a cryptographic anchor to the record, and a holder-granted retrieval right exercisable on lawful triggers. Every building block is already in this specification — attestation over the existing commitment `s` (§11.3), delegation as a signed revocable credential (§15.5), private status checks (§10.6), issuer-set privacy (§13.4), and consent receipts (§15.1); this section fixes how they compose. Adoption is optional; a deployment implementing it MUST implement all of it.

*Actors and accreditation.* The custodian MUST be a Tier-2/3 accredited issuer under the §15.6 trust framework, carrying the distinct accreditation class **CDD custodian**, and SHOULD be a qTSP or an entity itself subject to AML supervision. Ultimate CDD responsibility remains with the company under both reliance and outsourcing regimes and cannot be contracted away; the profile changes where the record physically lives, not who answers for it.
...
*Phase 2 — onboarding.* At the company, the holder presents the §10.3 proof extended with: (b′) the attestation was issued by a member of the accepted custodian set — proven without naming the custodian (§13.4), a real privacy gain since custodian choice is itself correlatable — together with the fresh `anchor_RP`, `ref_RP`, and the retrieval grant. The company stores `pid`, the proof transcript, the anchor, the locator, the grant, and the consent receipt. Its entire customer file is non-identifying: a breach yields pseudonyms and opaque pointers the attacker cannot resolve. One honest limit: the grant's envelope must name the custodian (endpoint and key) so the company can route a lawful retrieval, so the custodian is hidden from the verifiable transcript and from anyone the company shows it to, not from the company itself; custodian-oblivious retrieval via relay is an open extension.

*Phase 3 — monitoring.* Ongoing sanctions/PEP rescreening happens where the data lives. On a hit, record expiry, or SLA breach, the custodian flips the attestation status; the company's periodic private status check (§10.6, §13.5) surfaces the flag without the custodian learning who checked. The company then escalates — invoking Phase 4 or exiting the customer.

*Phase 4 — retrieval.* On a lawful trigger, the company opens a mutually authenticated BRC-103 session with the custodian and presents `ref_RP`, the retrieval grant, and evidence of the trigger basis. The custodian MUST verify the grant's signature chain against the enrolment binding (C.6), its status (§15.5), and the trigger evidence before resolving `ref_RP → R` — releasing the record, with `salt_RP`, to the company or directly to the authority. The recipient MUST verify `Open(anchor_RP, R, salt_RP)`; a mismatch is a custodian-integrity event. This gives the supervisor something conventional outsourced KYC cannot: cryptographic proof the record was neither altered nor substituted since the diligence was performed. Every disclosure event MUST be logged and appended to the consent receipt, and MAY be hash-anchored on-chain for non-repudiation (P12).
...
**14.2 Presentation request.** A relying party states what it needs — the required predicate set `P`, the minimum tier `t`, and whether unlinkability is required — as a presentation request (DIF Presentation Exchange / OpenID4VP query, or a BRC-103 certificate request). The presentation scope is **not** part of the request: `scope_RP` MUST be derived by the wallet, never accepted from the caller. A relying party that could choose its own scope string could collude with another to share one and receive identical `pid`s — defeating unlinkability by construction, since verifier identity is not otherwise an input to `pid = PRF(s, scope)`. Normatively: the wallet MUST compute `scope_RP = H_dst(canonical registered RP identifier)`, where the identifier is the relying party's registered identity in the applicable trust framework (its accredited `did:bsv` in the §15.6 registry for BRC-103; the registered client identifier under its trust scheme for OpenID4VP), authenticated during the §14.5 handshake; MUST reject a request whose asserted identity does not match the authenticated one; and MUST key the scope to the registered identifier, not to the RP's current keys, so RP key rotation does not silently re-scope pseudonyms. Deliberate scope sharing across legal entities (a corporate group operating one user base) is permitted only as a declared *sector scope* in the RP's registry entry, disclosed to the holder at consent time (§15.1). The wallet then computes the scoped pseudonym and the zero-knowledge presentation proof (§10, §13) that satisfies the request with the minimum disclosure consistent with it (P1).
...
Requirements: the grant MUST NOT contain `pid`, the root DID, or any holder attribute; `audience` binding MUST be enforced by the custodian (release only over a mutually authenticated session with the named key, §14.4); the `resolution` capability MUST be opaque to the company; the custodian MUST verify the enrolment binding inside `resolution` against its registered grant-verification key, the grant's `credentialStatus`, and the presented trigger evidence before release; and the disclosure event MUST be appended to the consent receipt (C.5), MAY be hash-anchored (P12). The `custodian` field is visible to the company by necessity (routing); it stays outside the §10.3 proof transcript, so the custodian's identity is not provable to third parties from the onboarding evidence (§10.10 Phase 2).
...
**F.1 States.** `unregistered` → `pending` → `active`, with terminal or superseding states `superseded`, `evicted`, `expired`, `revoked`. At most one `pending`-or-`active` record exists per tag at any time. When a record leaves `active`, the registry retains only an opaque record hash and the transition entry for auditability; the tag itself ceases to be queryable (§10.7, §15.3).
...
- (b) *chip freshness* — a fresh Active Authentication or Chip Authentication transcript, the chip's response computed over a value derived from the registration challenge (f), verified in-statement wherever the document supports AA/CA (§12); where it does not, the record MUST carry `uniqueness: none` and no personhood guarantee attaches, though the one-per-tag rule still applies;
...
| `active` → `superseded` | replacement registration under the same tag (tier upgrade §12/C.4; `s`-migration §8.10) | holder: fresh F.2 proof over the same document |
| `active` → `evicted` | accountable Tier-2/3 proofing over the same document (§12) | the evicting accredited issuer/qTSP, with an evidence reference logged |
| `active` → `expired` | document validity end (expiry from record metadata) | automatic |
| `active` → `revoked` | holder-initiated de-registration | holder: proof of opening of `C` (or the bound holder key) |

Eviction (`active` → `evicted`) is the only transition a party other than the holder or the clock can cause, and it MUST carry the evidence reference and the evicting party's accreditation identity so it is contestable (F.6).

**F.4 Concurrency and atomicity.** Registration attempts are serialized by the registry's append-only operation log: among concurrent submissions for one tag, the first log-ordered valid submission wins and later ones fail the collision check. The challenge (F.2(f)) makes each attempt atomic — it is spent by the transition, cannot authorize a second submission, and expires if the transition does not complete. There is no state in which two `active` records share a tag; an operator set that appends one anyway has provably misbehaved (F.5).
