...
**Public inputs** (agreed by prover and verifier): `root` — the current root of the anchored enrolment registry (a Poseidon Merkle tree over active credential commitments, §10); `scope` — a canonical hash of the relying party's *registered* identity, **derived by the wallet, never accepted from the verifier** (a verifier that could choose its own scope string could share it with another and collect identical `pid`s — Spec §14.2); `threshold` — the newest `dob` that is still ≥ 18 today, computed by the verifier (§2.1); `nonce` — the verifier's freshness challenge, derived from the session transcript (§8.5).
...
| Uniqueness (one account per scope, per document) | one active registration per document at the enrolment registry (`tag = PRF(DS_TAG, n)`, §10) ⇒ one `s` per document ⇒ one stable `pid` per scope | Sybil behaviour within a scope; re-enrolment with a fresh wallet (same document) |
...
- **Register.** At enrolment the wallet submits a registration record — the blinded commitment `C`, the uniqueness tag `tag = PRF(DS_TAG, n)`, and the Layer-D enrolment proof — to the registry overlay. **The tag stays off-chain** (Spec §10.7, §15.3): it is deterministic from chip data and therefore personal data, so the multi-party operator set holds it access-controlled and rate-limited, drops it when the registration terminates, and anchors on-chain only salted registry roots and opaque record hashes. Verification of the enrolment proof against the published DSC/CSCA trust root is registry-and-auditor-grade rather than universal — no operator can admit a registration without a valid proof, anyone can verify the anchored root history, but raw records are not world-readable. Never PII; `n` itself never appears anywhere.
...
**Correlation surfaces, stated plainly.** Two are accepted and must be documented wherever this registry is deployed. (1) *Enrolment-existence test:* `tag` is deterministically recomputable by anyone holding the passport's chip data, so a party with chip data **and registry query access** can test whether a document has enrolled — the bounded residual named in §5, now doubly gated: computing `tag` requires having *read the chip* (the `n` derivation includes the SOD hash, Spec §8.4), and testing it requires access to the off-chain, rate-limited registry (Spec §10.7) — nothing testable is on-chain. (2) *Registration timing:* the registration transaction's timing and funding are a linkage surface between the holder's wallet and their enrolment; mitigate with batched anchoring windows, randomized submission delay, and third-party transaction broadcast. (3) *Path retrieval:* asking the overlay for your Merkle path identifies your leaf; wallets SHOULD fetch paths via anonymous transport, request ranges/frontiers rather than single paths, or maintain the path incrementally from published tree updates.
...
- **Scope canonicalization — and who derives it.** `pid` is stable only if both sides derive `scope` identically (same canonical RP identifier, same hash) — but stability is the lesser half. The wallet MUST derive `scope` itself from the *authenticated, registered* RP identifier (Spec §14.2) and reject caller-supplied scope values: accepting the verifier's scope string lets colluding RPs share one and defeat cross-scope unlinkability entirely. Key it to the registered identifier, not the RP's current key, so RP key rotation doesn't re-scope pseudonyms.
...
- **Tier 1 (free self-read):** at enrolment, prove the full registration statement of Spec Appendix F.2 *inside* a circuit — passive authentication (the document-signer's RSA/ECDSA signature over the data groups, under an explicit algorithm and time policy), a **fresh AA/CA transcript** bound to the registry challenge (passive auth alone cannot detect a cloned chip, so a passive-only statement is insufficient wherever uniqueness is claimed), canonical `n`/`tag` derivation, and consistency with `C` — and submit the registration record of §10, verifiable by the registry operator set and auditors without an issuer. This is Layer D and the expensive step — amortized once, so the per-presentation proof stays exactly the cheap circuit built here. Production passport-ZK systems (ZKPassport, Self, OpenPassport, Rarimo) show it is shippable on phones; heterogeneous national signature algorithms are the real coverage cost.
