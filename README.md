...
> **Status:** v0.1 — draft for discussion. Not yet a ratified BRC or eIDAS profile.
...
- **`n` — personhood nullifier (the "uniKey").** A deterministic, one-way value derived
  from passport chip data. Anchors **per-document** uniqueness (one account per identity
  document — not provably one per natural person) and is *not* a secret.
...
- **Unlinkability is cryptographic and conditional.** It holds on the non-custodial,
  zero-knowledge presentation path. It does **not** extend to a custodial wallet, the network
  layer (IP/timing/device), or linkability *within* a published global-uniqueness set.
